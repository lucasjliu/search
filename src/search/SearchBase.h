...
namespace search
{
    ...
    //word hits in a doc
    //arranged by field and counted by frequency
    struct hit_t 
    {
        doc_id_t    docid;
    ...
    //word statistical information in the collection
    // and hits in each document
    struct tuple_t 
    {
        using VHIT = std::vector<hit_t>;
        using VHITS = std::unordered_map<doc_id_t, VHIT>;

        doc_id_t    ctf;    //term frequency in collection
        VHITS       vhits;  //list of hits mapped by docid

        tuple_t() :ctf(0) {}
        bool hasHit(doc_id_t i) const {return vhits.find(i) != vhits.end();}
        const VHIT& getHits(doc_id_t i) const {return vhits.find(i)->second;}
        bool operator==(const tuple_t& o) const {return ctf == o.ctf && vhits == o.vhits;}
    };
